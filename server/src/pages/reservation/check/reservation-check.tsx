...
export function reservationCheckPage(): MyHttpListener {
    return (req, user) => Promise.resolve(pageHtmlResponse({title: 'Check Reservation', user: user},
        <div class='center-container'>
            <form method='post' action='/reservation/check/reservation-check' class='form-container'>
                <input name='reservation_id' placeholder='Enter Reservation ID...'/>
...
export function reservationCheckReqList(con: Connection): MyHttpListener {
    return (req, user) => streamToString(req.body).then(bodyString => {
        ...
        return Promise.all([
            ...
            mysqlQuery(con, `SELECT s.id, s.row, s.number
                             FROM seat_reserved sr
                                      JOIN seat s on s.id = sr.seat_id
                             WHERE sr.reservation_id = ?`, [p['reservation_id']])
        ]).then(all => {
            const s = all[0];
            const sr = all[1]
            return pageHtmlResponse({title: 'Your Reservation', user: user},
                <div class='center-container'>
                            ...
                            <div class="flx-rw">
                                <div class="flx-rw">
                                    ...
                                    <div class="mr-rgt">
                                        ...
                                        <span id={sr[0].id}>{rowNumberToLetter(sr[0].row)}-{sr[0].number}</span>
                                    </div>
                                    <div class="mr-rgt">
                                        <strong>Reserved By:</strong>
                                        <span>{xmlEscape(s[0].reserved_by)}</span>
                                    </div>
                                    <form method="post" action="/cancel-reservation-token">
                                        <input type="text" name='id' value={p['reservation_id']} hidden/>
