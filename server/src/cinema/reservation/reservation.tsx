...
export function reservationPage(con: Pool): MyHttpListener {
    return async (req, user) => {
        ...
        const screening = await mysqlQuery<ScreeningListSqlResult>(con,
            `SELECT m.*,
                    s.screening_date,
        ...
        const availableSeats = await mysqlQuery<AvailableSeatsSQLResult>(con,
            `SELECT s.id,
                    s.row,
                    s.number
             FROM seat s
